...
export default function LoggedInLayout({
  ...
  return <DefaultLayout title={title}>
    ...
    <Grid container sx={{height: "100%"}} alignItems="stretch" flexWrap="nowrap">
      <Grid item sx={{padding: "20px", background: grey[50]}}>{button}</Grid>
      <Grid item flexGrow={1} sx={{overflowY: "scroll", padding: "20px 40px 10px", background: theme.palette.background.default}}>{children}</Grid>
    </Grid>
  </DefaultLayout>
}