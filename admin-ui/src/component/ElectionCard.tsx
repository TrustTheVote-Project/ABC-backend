...
export default function ElectionCard({
  ...
  return <Card>
    <Grid container justifyContent="space-between">
      ...
      <Grid item>
        <Typography variant="h3" sx={{textTransform: 'capitalize'}}>
          ...
          {` (${election.testCount || '0'} ${election.testCount === 1 ? 'test' : 'tests'})`}
        </Typography>
      </Grid>
    </Grid>
    <Grid container spacing={6}>
      ...
      {(election.electionStatus === ElectionStatus.live) && <Grid item xs={12}>
      ...
      {(!election.electionStatus || election.electionStatus === ElectionStatus.test || election.electionStatus === ElectionStatus.pending) && <Grid item xs={12}>
        <Grid spacing={1} container justifyContent="space-between">
          ...
          <Grid item xs={6} sm={6} md={4}>
            <Button disabled={election.electionId === currentElection?.electionId} onClick={async ()=>{
          ...
          <Grid item xs={6} sm={6} md={4}>
            <Button disabled={election.electionStatus !== ElectionStatus.test} onClick={async ()=>{
