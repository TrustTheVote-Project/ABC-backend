...
class Voter {
  ...
  static requiredPropertiesForImport = [
    "VIDN",
    "firstName",
    "lastName",
    "yearOfBirth",
  ...
  static filterProperties(attributes, master = Voter.objectProperties) {
  ...
  static filterConsumerProperties(attributes) {
    return Voter.filterProperties(attributes, Voter.consumerProperties);
  }

  consumerProperties() {
    return Voter.filterConsumerProperties(this.attributes);
  }
  ...
  static async findByVIDN(VIDN, election) {
    const data = await db.get(
      {
        VIDN_electionId_latMode: Voter.VIDNPlusIndex(VIDN, election),
      },
      tableName
    );
  ...
  */
  ...
  static VIDNPlusIndex(VIDN, election) {
    return db.multiColumnIndexKey([
      VIDN,
      election.attributes.electionId,
      election.attributes.latMode,
    ]);
  }

  static electionContextualizeIndex(columnValue, election) {
    return db.multiColumnIndexKey([
      columnValue,
      election.attributes.electionId,
      election.attributes.latMode,
  ...
  static SSN4HashPlusIndex(SSN4HashTruncated, lastName, yearOfBirth, election) {
    return db.multiColumnIndexKey([
      SSN4HashTruncated,
      lastName,
      yearOfBirth,
  ...
  static AddressPlusIndex(ZIP5, yearOfBirth, lastName, streetNumber, election) {
    return db.multiColumnIndexKey([
      ZIP5,
      //stateCode,
      //city,
  ...
  */
  ...
  static async findBySSN4Hash(
    ...
    let index = "SSN4HashPlus-index",
      key = "SSN4HashPlus_electionId_latMode",
  ...
  static async findByVoterDLNHash(
    ...
    let props = {
        ":dob": yearOfBirth,
        ":dlnhashtruncated": Voter.electionContextualizeIndex(
          IDnumberHashTruncated.toUpperCase(),
          election
        ),
      },
      constraints =
        "DLNhashTruncated_electionId_latMode=:dlnhashtruncated and yearOfBirth=:dob";
  ...
  static async findByVoterStateIdNumberHash(
    ...
    let props = {
        ":dob": yearOfBirth,
        ":sidnhashtruncated": Voter.electionContextualizeIndex(
          IDnumberHashTruncated.toUpperCase(),
          election
        ),
      },
      constraints =
        "stateIDNhashTruncated_electionId_latMode=:sidnhashtruncated and yearOfBirth=:dob";
  ...
  constructor(attributes) {
    this.attributes = attributes; //Voter.filterProperties(attributes);
  }
    ...
    if (summaryAttributes) {
      await db.transactWrite([
        ...
        },
        {
          ...
          keyValueObj: {
            VIDN_electionId_latMode: this.attributes.VIDN_electionId_latMode,
          },
          table: tableName,
        },
      ]);
  ...
  async incrementSession(type) {
    ...
    switch (type) {
      case "begin":
        ...
        break;
      case "complete":
        count += isNaN(this.attributes["completedN"])
          ? 0
          : parseInt(this.attributes["completedN"]);
        ...
        break;
      case "incomplete":
        count += isNaN(this.attributes["incompletedN"])
          ? 0
          : parseInt(this.attributes["incompletedN"]);
