...
class Voter {
  ...
  */
  static SSN4PlusIndex(SSN4, lastName, yearOfBirth) {
    return db.multiColumnIndexKey([SSN4, lastName, yearOfBirth]);
  }
  static AddressPlusIndex(ZIP5, stateCode, city, yearOfBirth, lastName) {
  ...
  */
  ...
  static async findBySSN4(SSN4, lastName, firstName, yearOfBirth) {
    let index = "SSN4Plus-index",
      key = "SSN4Plus",
      indexValue = Voter.SSN4PlusIndex(SSN4, lastName, yearOfBirth);
  ...
  static async findByVoterDLN(firstName, lastName, yearOfBirth, IDnumber) {
    let props = {
        ":dob": yearOfBirth,
        ":dln": IDnumber,
      },
      constraints = "DLN=:dln and yearOfBirth=:dob";
  ...
  static async findByVoterStateIdNumber(
    ...
    let props = {
      ...
      constraints = "stateIDN=:sidn and yearOfBirth=:dob";
  ...
  /*
  static async findByVoterAny(firstName, lastName, yearOfBirth, IDnumber) {
    const voter_vidn = await Voter.findByVIDN(IDnumber);

    const voter_ssn4 = await Voter.findBySSN4(
      IDnumber,
      lastName,
      firstName,
      yearOfBirth
    ...
    const voter_dln = await Voter.findByVoterDLN(
      firstName,
      lastName,
      yearOfBirth,
      IDnumber
    ...
    const voter_stateId = await Voter.findByVoterStateIdNumber(
      firstName,
      lastName,
      yearOfBirth,
      IDnumber
    ...
    //AM:  There should be some error checking for ambiguous result
    //     But adhering to the current MVP protocol of first match

    return Voter.coalesce(voter_vidn, voter_ssn4, voter_dln, voter_stateId);
  }
  */

  static async findByVoterIdNumber(firstName, lastName, yearOfBirth, IDnumber) {
