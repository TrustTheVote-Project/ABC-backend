  ...
  queryIndex(expressionAttributeValues, table, index, keyConditions) {
    return new Promise((resolve, reject) => {
      ...
      documentClient.query(params, function (err, data) {
        if (err) {
          console.log(
            `There was an error fetching the data for ${JSON.stringify(expressionAttributeValues)} on table ${table} with keyConditions ${JSON.stringify(keyConditions)}`,
            err
          );
          return reject(err);
        }
  ...
  get(keyValueObj, table) {
    return new Promise((resolve, reject) => {
      ...
      documentClient.get(params, function (err, data) {
        if (err) {
          console.log(
            `There was an error fetching the data for ${JSON.stringify(keyValueObj)} on table ${table} with key ${JSON.stringify(keyValueObj)}`,
            err
          );
          return reject(err);
        }
  ...
  getAll(attributesToGet, table) {
    return new Promise((resolve, reject) => {
      ...
      documentClient.scan(params, function (err, data) {
        if (err) {
          console.log(
            `There was an error fetching the data for ${JSON.stringify(attributesToGet)} on table ${table}`,
            err
          );
          reject(err);
        }
