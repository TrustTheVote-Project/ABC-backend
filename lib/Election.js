...
class Election {
  ...
  */
    ...
    if (documentState["fileCount"] == 1) {
      ...
      if (success) {
        await FileInProcessing.incrementProgress(
          objectId,
          FileInProcessing.processingStatus.complete,
  ...
  async loadVoterRecords(voterRecordsS3) {
    const s3Config = {
      Bucket: uploadBucket,
      Key: voterRecordsS3,
    };
    await this.loadRecords(s3Config);
  ...
  */
    ...
    let myReadPromise = new Promise((resolve, reject) => {
      rl.on("line", async (line) => {
        ...
        await this.insertVoterRecord(
          this.prepareVoterRecordForDB(JSON.parse(line))
        );
      });
      rl.on("error", () => {
        console.log("error");
  ...
  async setElectionVoters(documentState) {
    if (documentState["fileCount"] != 1) {
      return [false, "Multiple files uploaded"];
    }
    if (
    ...
    if (response) {
      try {
        ...
        await this.loadVoterRecords(s3OutputKey);
  ...
  async setElectionBallots(documentState) {
