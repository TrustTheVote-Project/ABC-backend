...
class Election {
  ...
  */
  ...
  static validateEDFSemantics = async (edfJSON) => {
    ...
    const districtForEveryContest = contests.every((contest) =>
      precinctIds.contains(contest.ElectionDistrictId)
    );
    ...
    [valid, messageArray] = validate(
      districtForEveryContest,
      "No district matching contest electionDistrict",
      valid,
      messageArray
    );
