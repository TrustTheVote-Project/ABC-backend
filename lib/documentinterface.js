...
class DocumentInterface {
  ...
  static async documentFileProvisioned(uploadKey) {
    ...
    await FileInProcessing.set(uploadKey + "_uploadState", uploadState);
  }

  static async documentSingleFileUploaded(uploadKey) {
    const currentState = await FileInProcessing.get(uploadKey + "_uploadState");
    const uploadState = currentState || {};
    uploadState["fileCount"] = 1;
    uploadState["files"] = { uploadKey: uploadKey };
  ...
  static async documentFileUnzipping(uploadKey, filename, newKey) {
    ...
    uploadState["fileCount"] = Object.keys(uploadState["files"])?.length ?? 0;
    uploadState["status"] = "processing";
    await FileInProcessing.set(uploadKey + "_uploadState", uploadState);
  }

  static async documentFileUnzippingComplete(uploadKey) {
    const uploadState = await FileInProcessing.get(uploadKey + "_uploadState");
    uploadState["status"] = "ready";
    await FileInProcessing.set(uploadKey + "_uploadState", uploadState);
  }
  static async getDocumentState(uploadKey) {
    return await FileInProcessing.get(uploadKey + "_uploadState");
  }
