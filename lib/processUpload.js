...
const processUpload = async (Bucket, uploadKey) => {
  if (uploadKey.endsWith(".zip")) {
    return await unzip(Bucket, uploadKey);
  } else {
    console.log(uploadKey + " single file uploaded. Documenting....");
    await DocumentInterface.documentSingleFileUploaded(uploadKey);
    return uploadKey;
  }
};
